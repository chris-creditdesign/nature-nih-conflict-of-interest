...
for i, v in enumerate(set_org_names):
	my_dict = {'ORG_NAME':v, "PROJECT":0, "FCOI":0}
...
for i, v in enumerate(list_summed_orgs):
	for y, x in enumerate(list_all_orgs):
		if x['ORG_NAME'] == v['ORG_NAME']:
			...
			v['PROJECT'] += 1
			if x['FCOI_ID'] != "":
				print "We got an FCOI"
				v['FCOI'] += 1

fieldnames = ('ORG_NAME', 'PROJECT', 'FCOI')
...
for y, x in enumerate(list_summed_orgs):
	writer.writerow({ 'ORG_NAME':x['ORG_NAME'], 'PROJECT':x['PROJECT'], 'FCOI':x['FCOI']})
